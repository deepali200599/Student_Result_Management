...
class studentClass:
    ...
    def __init__(self,root):
        ...
        self.root.geometry("1300x480+80+170")
        self.root.config(bg="white")
        self.root.focus_force()
        #====title=====
        
        title=Label(self.root,text="Manage Student Details",font=("goudy old style",20,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1280,height=35)
        #=====variables======
        ...
        txt_pin=Entry(self.root,textvariable=self.var_pin,font=("goudy old style",15,"bold"),bg="lightyellow").place(x=570,y=220,width=120)
        ...
        lbl_admisson=Label(self.root,text="Admisson",font=("goudy old style",15,"bold"),bg="white").place(x=360,y=140)
        lbl_course=Label(self.root,text="Course",font=("goudy old style",15,"bold"),bg="white").place(x=360,y=180)

        #================entry field coloumn2=====================
        self.course_list=[]
        #==============function callll==========================
        self.fetch_course()
        ...
        txt_admisson=Entry(self.root,textvariable=self.var_a_date,font=("goudy old style",15,"bold"),bg="lightyellow").place(x=480,y=140,width=200)
        ...
        self.btn_add=Button(self.root,text="Save",font=("goudy old style",15,"bold"),bg="#31b0ac",fg="white",cursor="hand2",command=self.add)
        self.btn_add.place(x=170,y=400,width=110,height=40)
        
        self.btn_update=Button(self.root,text="Update",font=("goudy old style",15,"bold"),bg="#6bb742",fg="white",cursor="hand2",command=self.update)
        self.btn_update.place(x=300,y=400,width=110,height=40)
        
        self.btn_delete=Button(self.root,text="Delete",font=("goudy old style",15,"bold"),bg="#dcd033",fg="white",cursor="hand2",command=self.delete)
        self.btn_delete.place(x=420,y=400,width=110,height=40)
        
        self.btn_clear=Button(self.root,text="Clear",font=("goudy old style",15,"bold"),bg="#e35b6f",fg="white",cursor="hand2",command=self.clear)
      ...
      #======search=======
        ...
        btn_searh=Button(self.root,text="Search",font=("goudy old style",15,"bold"),bg="#7d3adb",fg="white",cursor="hand2",command=self.search).place(x=1070,y=60,width=120,height=28)
        ...
        self.CourseTable=ttk.Treeview(self.C_frame,columns=("roll","name","email","gender","dob","contact","admisson","course","state","city","pin","address"),xscrollcommand=scrollx.set,yscrollcommand=scrolly.set)
        ...
        self.CourseTable.heading("admisson",text="Admisson")
        ...
        self.CourseTable.column("admisson",width=100)
    ...
    def delete(self):
        ...
        try:
          if self.var_rollno.get()=="":
              messagebox.showerror("Error ","Roll no should be reuired",parent=self.root)
          else:
              cur.execute("select * from student where name=?",(self.var_rollno.get(),))
              row=cur.fetchone()
              if row ==None:
                   messagebox.showerror("Error ","Please select student form the list first",parent=self.root)
              else:
                  op=messagebox.askyesno('confirm','Do you really want to delete?',parent=self.root)
                  if op==True:
                      cur.execute("delete from student where roll=?",(self.var_rollno.get(),))
                      con.commit()
                      messagebox.showinfo("Delete","Student deleted Sucessfullly",parent=self.root)
                      self.clear()
        except Exception as ex:
          messagebox.showerror("Error",f"Error due to {str(ex)}")
        ...
        try:
          if self.var_rollno.get()=="":
              ...
              if row !=None:
                  ...
                  cur.execute("insert into student(roll,name,email,gender,dob,contact,admisson,course,state,city,pin,address) values(?,?,?,?,?,?,?,?,?,?,?,?)",(
                      self.var_rollno.get(),
                      self.var_name.get(),
                      self.var_email.get(),
                      self.var_gender.get(),
        ...
        try:
          if self.var_rollno.get()=="":
              ...
              if row ==None:
                  ...
                  cur.execute("update  student set name=?,email=?,gender=?,dob=?,contact=?,admisson=?,course=?,state=?,city=?,pin=?,address=? where roll=?",(
