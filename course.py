...
class CourseClass:
    def __init__(self,root):
        ...
        title=Label(self.root,text="Manage Course Details",font=("goudy old style",20,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1280,height=35)
        #=====variables======
        ...
        lbl_discription=Label(self.root,text=" Discription",font=("goudy old style",15,"bold"),bg="white").place(x=10,y=180)
        #======entry fields====
        ...
        self.txt_discription.place(x=150,y=180,width=500,height=150)
    ...
    def delete(self):
        ...
        try:
          if self.var_course.get()=="":
              messagebox.showerror("Error ","Course name should be reuired",parent=self.root)
          else:
              cur.execute("select * from course where name=?",(self.var_course.get(),))
              row=cur.fetchone()
              if row ==None:
                   messagebox.showerror("Error ","Please select course form the list first",parent=self.root)
              else:
                  op=messagebox.askyesno('confirm','Do you really want to delete?',parent=self.root)
                  if op==True:
                      cur.execute("delete from course where name=?",(self.var_course.get(),))
                      con.commit()
                      messagebox.showinfo("Delete","Course deleted Sucessfullly",parent=self.root)
                      self.clear()
        except Exception as ex:
          messagebox.showerror("Error",f"Error due to {str(ex)}")
    ...
    def get_data(self,ev):
        ...
        #self.txt_discription.set(row[4])
        self.txt_discription.delete('1.0',END)
